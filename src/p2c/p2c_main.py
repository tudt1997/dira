...
libc = npct.load_library('libmyfunc', "/home/nvidia/catkin_ws/src/dira/src/p2c")
...
libc.get_center_point_left_and_right.argtypes = [array_2d_int, c_float, c_float, c_int, POINTER(c_int), POINTER(c_int), c_int, c_int, c_float, c_float, c_int]
...
def get_center_point_left_and_right(img, roi, future_roi, flag, left_restriction, right_restriction, total_time, time_threshold, mode):
    p_x = pointer(c_int())
    p_y = pointer(c_int())
    libc.get_center_point_left_and_right(img, roi, future_roi, flag, p_x, p_y, left_restriction, right_restriction, total_time, time_threshold, mode)
    return p_y.contents.value, p_x.contents.value
